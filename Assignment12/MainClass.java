...
public class MainClass
{
    public static void main(String args[])
    {
        ...
        List departments = studentList.stream()
                .filter(distinctByKey(s -> s.engDepartment))
                .map(d->d.engDepartment)
                .collect(Collectors.toList());
        System.out.println(departments);

        System.out.println("\nGet the details of all students who have enrolled after 2018.");
        studentList.stream()
                .filter(s->s.year_of_enrollment>2018)
                .map(n->"NAME : "+(n.name)+", ID : "+(n.id)+", DEPT. : "+(n.engDepartment)+", GENDER : "+(n.gender)+
                        ", AGE : "+(n.age)+", PERTILLDATE : "+(n.perTillDate)+", YEAR OF ENROLLMENT : "+(n.year_of_enrollment))
                .forEach(System.out::println);

        System.out.println("\nGet the names of all male student in the computer sci department.");
        List male=studentList.stream()
                .filter(s->s.gender.equals("Male") && s.engDepartment.equals("Computer Science"))
                .map(n->n.name)
                .collect(Collectors.toList());
        ...
        Optional<String> d= studentList.stream()
                .max(Comparator.comparing(a -> a.perTillDate))
                .map((Student n)->"NAME : "+(n.name)+", ID : "+(n.id)+", DEPT. : "+(n.engDepartment)+", GENDER : "+(n.gender)+
                        ", AGE : "+(n.age)+", PERTILLDATE : "+(n.perTillDate)+", YEAR OF ENROLLMENT : "+(n.year_of_enrollment));
        System.out.println(d);

        System.out.println("\nCount the number of students in each department");
        Map<String, Long> studentsByDepartment = studentList.stream()
        ...
        Map<String, Double> avgPercentageInEachDepartment = studentList.stream()
        ...
        System.out.println("\nGet the name of youngest male student in the Electronic department ");
        Map<Object, Optional<Student>> youngElectronic = studentList.stream()
                .filter(s->s.engDepartment.equals("Electronic"))
                .collect(Collectors.groupingBy(s->s.engDepartment, Collectors.minBy(Comparator.comparing(a->a.age))));
        ...
        Map<String, Long> computerStudents = studentList.stream()
